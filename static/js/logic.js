...
function next() {
    ...
    var audio = $("#audioCtrl");
    $("#appAudio").attr({ src: './images/' + subject + '/' + (lesson) + '/' + (chapter) + '/' + scrollId + '.mp3' });
    audio[0].pause();
    if (audioCheck) {
...
function prev() {
    if (scrollId > 1) {
        ...
        $("#main_image").attr('src', './images/' + subject + '/' + (lesson) + '/' + (chapter) + '/' + scrollId + '.jpg');
        $("#appAudio").attr({ src: './images/' + subject + '/' + (lesson) + '/' + (chapter) + '/' + scrollId + '.mp3', autoplay: true });
        var audio = $("#audioCtrl");
        $("#appAudio").attr({ src: './images/' + subject + '/' + (lesson) + '/' + (chapter) + '/' + scrollId + '.mp3' });
        audio[0].pause();
        if (audioCheck) {
